...
import { V1_ENTITY_OPTIONS, getServiceByKey } from '../data/entityServiceCatalog';
...
const ClientListPage: React.FC = () => {
    ...
    const handleSelectClient = (clientId: string) => {
        ...
        if (flow === 'upload') {
            ...
            setPage(selectedService?.route || selectedServiceId);
        } else {
                    ...
                    <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md space-y-4">
                        ...
                        <div>
                            ...
                            <select value={entityType} onChange={e => setEntityType(e.target.value)} className="input w-full mt-1">
                                ...
                                {V1_ENTITY_OPTIONS.map(type => <option key={type.id} value={type.id}>{type.name}</option>)}
                            </select>
                        </div>
