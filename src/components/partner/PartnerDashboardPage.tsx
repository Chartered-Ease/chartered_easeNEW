...
import { V1_ENTITY_OPTIONS } from '../../data/entityServiceCatalog';
                            ...
                            {agentClients.map(client => (
                                ...
                                >
                                    <div>
                                        ...
                                        <span className="inline-block mt-2 bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded">{V1_ENTITY_OPTIONS.find(e => e.id === client.entityType)?.name || client.entityType}</span>
                                    </div>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 animate-fade-in">
                    <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md space-y-4">
                        ...
                        <select value={entityType} onChange={e => setEntityType(e.target.value)} className="input w-full">
                            ...
                            {V1_ENTITY_OPTIONS.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                        </select>
