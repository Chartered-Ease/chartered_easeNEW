...
const TrackStatusDashboardPage: React.FC = () => {
    ...
    const submissionsWithDetails = useMemo(() => {
        ...
        return submissions.filter(sub => agentClientIds.has(sub.clientId)).map(sub => {
            const client = agentClients.find(c => c.id === sub.clientId);
    ...
    }, [submissions, agentClients, tasks, associates]);

    const serviceOptions = useMemo(() => {
        return Array.from(new Set(submissionsWithDetails.map(sub => sub.service))).sort();
            ...
            <div className="bg-white p-4 rounded-lg shadow-md mb-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <select name="client" value={filters.client} onChange={handleFilterChange} className="input">
                        ...
                        {agentClients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </select>
