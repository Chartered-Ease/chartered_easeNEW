...
interface ClientContextType {
    ...
    findClientsByEmail: (email: string) => Client[];
    findClientsForUser: (user: { mobileNumber?: string; email?: string; firebaseUid?: string }) => Client[];
...
const mergeDocuments = (existingDocuments: Document[] = [], newDocuments: Document[] = []) => {
    const seen = new Set(existingDocuments.map(doc => `${doc.type}::${doc.fileName}`));
    const uniqueNewDocuments = newDocuments.filter(doc => {
    ...
    return [...existingDocuments, ...uniqueNewDocuments];
};
...
export const ClientProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const findClientByMobile = useCallback((mobileNumber: string): Client | undefined => {
        return clients.find(c => c.mobileNumber === mobileNumber);
    }, [clients]);

    const findClientsByMobile = useCallback((mobileNumber: string): Client[] => {
        return clients.filter(c => c.mobileNumber === mobileNumber);
    }, [clients]);

    const findClientsByEmail = useCallback((email: string): Client[] => {
        const normalizedEmail = email.trim().toLowerCase();
        if (!normalizedEmail) return [];
        return clients.filter(c => c.email?.trim().toLowerCase() === normalizedEmail);
    }, [clients]);

    const findClientsForUser = useCallback((user: { mobileNumber?: string; email?: string; firebaseUid?: string }): Client[] => {
        const normalizedMobile = user.mobileNumber?.trim();
        const normalizedEmail = user.email?.trim().toLowerCase();
        const firebaseUid = user.firebaseUid?.trim();

        return clients.filter(client => {
            if (normalizedMobile && client.mobileNumber === normalizedMobile) return true;
            if (normalizedEmail && client.email?.trim().toLowerCase() === normalizedEmail) return true;
            if (firebaseUid && client.createdBy?.type === 'customer' && client.createdBy.id === firebaseUid) return true;
            return false;
        });
    }, [clients]);
    ...
    const addDocumentsToClient = (clientId: string, newDocuments: Document[]) => {
        const updatedClients = clients.map(client => {
            if (client.id === clientId) {
                return {
                    ...client,
                    documents: mergeDocuments(client.documents || [], newDocuments)
                };
            }
            return client;
        });
    ...
    const processServiceApplication = useCallback((
        ...
        const updatedClients = clients.map(client => {
            if (client.id === clientId) {
                client.documents = mergeDocuments(client.documents || [], newDocuments);
    ...
    return (
        <ClientContext.Provider value={{ clients, isLoading, addClient, getClient, getClientsForAgent, findClientByMobile, findClientsByMobile, findClientsByEmail, findClientsForUser, updateClient, updateClientProfileType, updateClientGstCredentials, updateClientItrCredentials, addDocumentsToClient, processServiceApplication }}>
            {children}
